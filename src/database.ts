...
const listMemoriesStatementCache = new Map<string, Database.Statement>();
const searchMemoriesStatementCache = new Map<string, Database.Statement>();
...
export function listMemories(args: ListMemoryArgs): Memory[] {
  ...
  let stmt = listMemoriesStatementCache.get(cacheKey);
  ...
  if (!stmt) {
    ...
    listMemoriesStatementCache.set(cacheKey, stmt);
  }
...
export function searchMemories(args: SearchMemoryArgs): SearchResult[] {
  ...
  let stmt = searchMemoriesStatementCache.get(cacheKey);
  ...
  if (!stmt) {
    ...
    stmt = database.prepare(sql);
    searchMemoriesStatementCache.set(cacheKey, stmt);
  }
...
export function fallbackSearch(args: SearchMemoryArgs): SearchResult[] {
  ...
  // Create a unique cache key based on the structure of the query (number of words) and filters
  const cacheKey = `fallback-${words.length}-${!!args.area}-${!!args.project}`;
  let stmt = searchMemoriesStatementCache.get(cacheKey);
...
export function closeDatabase(): void {
  if (db) {
    ...
    listMemoriesStatementCache.clear();
    searchMemoriesStatementCache.clear();
    getMemoryStmt = null;
