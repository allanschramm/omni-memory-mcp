...
// Bolt: Performance Optimization
// Dynamic statement caches for search and list operations
const listStmtCache = new Map<string, Database.Statement>();
const searchStmtCache = new Map<string, Database.Statement>();
...
export function listMemories(args: ListMemoryArgs): Memory[] {
  ...
  let stmt = listStmtCache.get(cacheKey);
  ...
  if (!stmt) {
    ...
    listStmtCache.set(cacheKey, stmt);
  }
...
export function searchMemories(args: SearchMemoryArgs): SearchResult[] {
  ...
  let stmt = searchStmtCache.get(cacheKey);
  ...
  if (!stmt) {
    ...
    stmt = database.prepare(sql);
    searchStmtCache.set(cacheKey, stmt);
  }
