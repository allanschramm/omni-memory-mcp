...
## 2024-05-27 - Cached dynamic better-sqlite3 statements using permutation keys
**Learning:** Calling `better-sqlite3`'s `database.prepare(sql)` repeatedly inside highly-frequent query functions (like `listMemories` and `searchMemories`) that construct dynamic SQL strings incurs a measurable recompilation overhead, particularly as the database scales. Because the query shapes in these list and search operations are determined by simple combinations of optional boolean flags (e.g. `!!args.area`, `!!args.project`), the total number of unique queries is small.
**Action:** Always utilize a module-level `Map` cache and concise string keys generated from the presence or absence of these optional filters (e.g. `const cacheKey = \`\${!!args.area}-\${!!args.project}\`;`) to store the dynamically prepared `better-sqlite3` `Statement`s. Crucially, remember to explicitly call `.clear()` on these caching Maps within teardown procedures (`closeDatabase` and `resetDatabase`) to prevent memory leaks and dangling statement handles across environments or tests.
