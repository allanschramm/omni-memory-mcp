...
## 2026-05-11 - Dynamic Statement Caching for Parameterized Search and List
**Learning:** While static queries without variations can be cached in a single module-level variable, queries that are dynamically assembled based on parameter presence (like `area`, `project`, `tag`) or list lengths (like `words.length`) still incur redundant `database.prepare()` compilation overhead if they are assembled dynamically on every call. Because the number of possible SQL string variations is very small (e.g., ^3 = 8$ for `listMemories`), compiling the SQL on every function call generates unnecessary CPU overhead.
**Action:** For dynamically generated queries with a small, bounded number of permutations, use a module-level `Map<string, Database.Statement>` to cache the prepared statements. Construct a cache key string from the permutation flags (e.g., `${!!args.area}-${!!args.project}`). Like single-statement caches, explicitly clear the `Map` objects during `closeDatabase` and `resetDatabase` lifecycles to prevent memory leaks.
