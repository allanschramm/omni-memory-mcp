...
## 2024-05-18 - Caching Dynamic SQLite Queries
**Learning:** In highly dynamic functions like `listMemories` and `searchMemories` (or `fallbackSearch`), using SQLite's `db.prepare()` every time with varying query strings incurs unnecessary SQL compilation overhead. Simply using string combination to create the SQL string dynamically inside the method bypasses static variable caching.
**Action:** When working with dynamic SQL queries based on optional parameters, map those parameter permutations (e.g., using flags like `!!args.area`) to a structured cache key string (e.g. `${!!args.area}-${!!args.project}-${!!args.tag}`). Use this string to look up and store the compiled statement in a module-level `Map`, drastically reducing query compilation overhead across repeated calls while preserving safe variable substitution via parameterized inputs (`?`). Ensure to clear these module-level maps during the database tear-down lifecycle (`closeDatabase` and `resetDatabase`).
